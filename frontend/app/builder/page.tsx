...
function buildPDFHtml(data: ResumeData, photo: string | null): string {
  ...
  const eduHTML = eduRows.length
    ...
    <div style="margin-bottom:14px">
      ...
      ${eduRows
        .map(
          (e, i) => `
        <div style="margin-top:${i === 0 ? "0" : "9px"};page-break-inside:avoid">
  ...
  const expHTML = expRows.length
    ...
    <div style="margin-bottom:14px">
      ...
      ${expRows
        .map(
          (e, i) => `
        <div style="margin-top:${i === 0 ? "0" : "10px"};page-break-inside:avoid">
          <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:1px">
  ...
  // ── HEADER: always centered; photo floats right via absolute positioning ──
  const headerHTML = hasPhoto
    ? `<div style="position:relative;text-align:center;border-bottom:2.5px solid #000;
                  padding-bottom:12px;margin-bottom:14px;page-break-inside:avoid">
        <div style="position:absolute;right:0;top:0;width:100px;height:126px;
                    border:1.5px solid #000;overflow:hidden;background:#f0f0f0">
          <img src="${photo}" crossorigin="anonymous"
               style="width:100%;height:100%;object-fit:cover;object-position:center top;display:block"/>
        </div>
        <div style="padding-right:114px">
          <h1 style="font-family:${F};font-size:26pt;font-weight:bold;text-transform:uppercase;
                     letter-spacing:1px;line-height:1.05;margin:0 0 5px 0;text-align:center">
            ${data.full_name || "YOUR NAME"}
          </h1>
          ${contacts ? `<div style="font-family:${F};font-size:10pt;color:#222;line-height:1.7;text-align:center">${contacts}</div>` : ""}
          <div style="text-align:center">${socialLines}</div>
        </div>
      </div>`
    : `<div style="text-align:center;border-bottom:2.5px solid #000;
                  padding-bottom:12px;margin-bottom:14px;page-break-inside:avoid">
        <h1 style="font-family:${F};font-size:26pt;font-weight:bold;text-transform:uppercase;
                   letter-spacing:1px;line-height:1.05;margin:0 0 5px 0">
          ${data.full_name || "YOUR NAME"}
        </h1>
...
<body>
  <div id="cv-root" style="width:794px;padding:40px 60px;background:white;font-family:${F};color:#000">
    ${headerHTML}
