...
async function generateCVWithAI(
  ...
  try {
    ...
    const withIds: ResumeData = {
      ...
      certifications: (d.certifications ?? []).map((e) => ({
        ...e,
        id: uid(),
      })),
...
export default function Builder() {
  ...
  function makeU<T extends { id: number }>(key: keyof ResumeData, blank: () => T) {
    const add = () =>
      setResume((p) => ({ ...p, [key]: [...(p[key] as unknown as T[]), blank()] }));
    const remove = (id: number) =>
      setResume((p) => ({ ...p, [key]: (p[key] as unknown as T[]).filter((x) => x.id !== id) }));
    const upd = (id: number, f: keyof T, v: string) =>
      setResume((p) => ({
        ...p,
        [key]: (p[key] as unknown as T[]).map((x) => (x.id === id ? { ...x, [f]: v } : x)),
      }));
    ...
    try {
      ...
      if (eduR.length) {
        ...
        for (const e of eduR) {
          ...
          if (e.degree || e.cgpa)
            ch.push(new Paragraph({ spacing: { after: 40 }, children: [tx(e.degree, undefined, true), ...(e.cgpa ? [tx(`\tCGPA: `), bd(e.cgpa)] : [])] }));
        }
      }
      ...
      if (prR.length) {
        ...
        for (const p of prR) {
          ...
          if (p.link)
            ch.push(new Paragraph({ spacing: { after: 20 }, children: [tx("Link: "), new ExternalHyperlink({ link: p.link, children: [new TextRun({ text: p.link, size: 19, font: TN, color: "1a56db", underline: {} })] })] }));
      ...
      if (skR.length) {
        ch.push(SH("Skills"));
        for (const s of skR)
          ch.push(new Paragraph({ spacing: { after: 30 }, children: [...(s.category ? [bd(`${s.category}: `)] : []), tx(s.skills)] }));
      }
      ...
      if (ceR.length) {
        ...
        for (const c of ceR)
          ch.push(new Paragraph({ spacing: { after: 30 }, children: [bd(c.name), ...(c.issuer ? [tx(` — ${c.issuer}`, undefined, true)] : []), ...(c.date ? [new TextRun({ text: `\t${c.date}`, bold: true, size: 20, font: TN })] : [])] }));
      }
      ...
      if (exR.length) {
        ...
        for (const e of exR)
          ch.push(new Paragraph({ spacing: { after: 30 }, children: [bd(`${e.label}: `), tx(e.value)] }));
      }
  ...
  return (
    <>
            ...
            >
              <motion.div className="ai-modal" initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }}>
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-10 h-10 rounded-xl bg-indigo-500/15 flex items-center justify-content:center">
                ...
                <div className="flex gap-3">
                  <button onClick={() => setShowAIModal(false)} className="flex-1 py-3 rounded-xl border border-slate-700 text-slate-400 text-sm font-semibold hover:border-slate-600 transition-all">
                    Cancel
                  </button>
                  <button
                    onClick={async () => {
        ...
        <div className="w-full lg:w-1/2 p-4 lg:p-6 overflow-y-auto max-h-screen scrollbar-hide border-r border-white/5">
          ...
          <div className="mb-3 p-2.5 rounded-xl bg-indigo-500/8 border border-indigo-500/20 flex items-center gap-3">
            ...
            <p className="text-slate-400 text-xs leading-relaxed">
              <span className="text-indigo-400 font-bold">New:</span> Click <span className="text-white font-bold">AI Build</span> — describe yourself and AI writes your entire CV free.
            </p>
          </div>
          ...
          <div className="sec-box">
            <div className="flex items-center justify-between">
              ...
              <button className="aibtn mb-3 ml-2 shrink-0" disabled={!!aiLoading} onClick={() => aiImprove(resume.summary, "professional summary", (v) => setField("summary", v), "summary")}>
                ...
                {resume.experience.map((e, idx) => (
                  <div key={e.id}>
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button className="aibtn" disabled={!!aiLoading} onClick={() => aiImprove(e.bullets, "work experience", (v) => exp.upd(e.id, "bullets", v), `exp-${e.id}`)}>
                ...
                {resume.projects.map((p, idx) => (
                  <div key={p.id}>
                    ...
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button className="aibtn" disabled={!!aiLoading} onClick={() => aiImprove(p.bullets, "project description", (v) => proj.upd(p.id, "bullets", v), `proj-${p.id}`)}>
      ...
      <div style={{ position: "absolute", left: "-9999px", top: 0, width: 794, background: "white", zIndex: -1 }}>
